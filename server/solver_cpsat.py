...
# Configure logging at the beginning of the file or function
    ...
    try:
        ...
        for s in subjects:
            ...
            for cid in s.get('assignedClassIds', []):
                ...
                # --- DEBUG PRINT --- (changed to logging.debug)
                logging.debug(f"Subject: {s['name']} ({sid}), Class: {c['name']} ({cid})")
                logging.debug(f"requiredTeacherCount: {req_teachers}")
                logging.debug(f"eligible_teachers (t_ids): {t_ids}")
                # --- END DEBUG PRINT ---
                ...
                # --- DEBUG PRINT --- (changed to logging.debug)
                logging.debug(f"teacher_combos: {teacher_combos}")
                # --- END DEBUG PRINT ---
                # ...
        ...
        # started = time.time() # Remove this line
        ...
        logging.error(f"Solver internal error: {e}", exc_info=True)
        return {
            'schedule': {},
            'stats': {
                'notes': [f"Solver internal error: {e}"],
                'status': 'ERROR',
