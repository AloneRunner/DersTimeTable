    ...
    self.onmessage = (evt) => {
      ...
      class LocalSolver {
        ...
        preflightValidate() {
          ...
          if (missing.length) {
            issues.push("Öğretmen eşleşmesi yok: " + missing.slice(0,6).join(", ") + (missing.length>6?" …":""));
          }
        ...
        solve() {
          ...
          while (!this.isTimedOut()) {
            ...
            for (let i=units.length-1;i>0;i--) { const j=(Math.random()*(i+1))|0; const t=units[i]; units[i]=units[j]; units[j]=t; }
            this.greedySeed(units);

            let ok = this.repairLoop(units);
            if (!ok && !this.isTimedOut()) {
        ...
        objectiveScore() {
          ...
          for (let d=0; d<DAYS; d++) score += 1*this.teacherSpreadPenalty(d);
          return score;
        }
        ...
        ruinAndRecreate(maxRemove: number) {
          ...
          // Collect all assignment starts
          const starts: Array<{classId:string, day:number, hour:number, span:number, subjectId:string}> = [];
          for (const classId of Object.keys(this.schedule)) {
            ...
            for (let d=0; d<DAYS; d++) {
              ...
              for (let h=start; h<end; h++) {
                ...
                starts.push({ classId, day:d, hour:h, span, subjectId:a.subjectId });
              }
            }
          }
          ...
          // Shuffle and pick K
          for (let i=starts.length-1;i>0;i--){ const j=(Math.random()*(i+1))|0; const t=starts[i]; starts[i]=starts[j]; starts[j]=t; }
          const picked = starts.slice(0, Math.min(maxRemove, starts.length));
        ...
        tabuOptimize() {
          ...
          for (let iter=0; iter<maxIter; iter++) {
            ...
            for (let i=neigh.length-1;i>0;i--) { const j=(Math.random()* (i+1))|0; const t=neigh[i]; neigh[i]=neigh[j]; neigh[j]=t; }
