...
export const SubjectForm: React.FC<{
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {step === 1 && renderStep1()}
      {step === 2 && renderStep2()}
      {step === 3 && (subject as any).assignedClassIds.length > 0 && data.teachers.length > 0 && renderStep3()}
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        {step > 1 && (
          <button type="button" onClick={() => setStep(step - 1)} className="px-4 py-2 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-700">
            Geri
          </button>
        )}
        {step < 3 && (
          <button type="button" onClick={() => setStep(step + 1)} className="px-4 py-2 rounded-md bg-sky-500 hover:bg-sky-600 text-white">
            İleri
          </button>
        )}
        {step === 3 && (
