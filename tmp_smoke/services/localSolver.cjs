...
exports.solveTimetableLocally = void 0;
    ...
    function solverWorker() {
        ...
        self.onmessage = (evt) => {
            ...
            function normalizeName(str) {
                ...
                if (s === 'ing' || s === 'ingilizce')
                    s = 'ingilizce';
                if (s === 'mat' || s === 'matematik')
                    s = 'matematik';
                return s;
            }
            class LocalSolver {
                ...
                data;
                options; // Using any because it's extended with defaults in the constructor.
                teacherById;
                subjectById;
                classroomById;
                schedule;
                ...
                constructor(data, options) {
                    ...
                    this.maxDailyHours = (allHours.length ? Math.max(...allHours) : 8) || 8;
                    this.stats = {
                ...
                resetForRestart() {
                    ...
                    (this.data.duties || []).forEach(duty => {
                        if (this.teacherOccupied[duty.teacherId]) {
                            ...
                            if (d >= 0 && d < DAYS && h >= 0 && h < H)
                                this.teacherOccupied[duty.teacherId][d][h] = true;
                        }
                    });
                }
                // ---- Zaman ----
                ...
                defaultMaxConsec(level) { return level === 'Lise' ? 3 : 2; }
                getRunLimitFor(classroomId, subjectId) {
                    const cls = this.classroomById.get(classroomId);
                    const subj = this.subjectById.get(subjectId);
                    // Ders özelinde override
                    if (typeof subj?.maxConsec === "number")
                        return subj.maxConsec;
                    // 3'lü blok tanımlıysa 3'e izin ver
                    if ((subj?.tripleBlockHours | 0) >= 3)
                        return 3;
                ...
                getAllowedWindow(classroom, day) {
                ...
                lessonKey(classroomId, subjectId, isBlock, span) {
                ...
                initialize() {
                    ...
                    (this.data.teachers || []).forEach(teacher => {
                        (teacher.branches || []).forEach(branch => {
                            if (!this.teacherSubjectMap.has(branch))
                                this.teacherSubjectMap.set(branch, []);
                            this.teacherSubjectMap.get(branch).push(teacher.id);
                            const nb = normalizeName(branch);
                            if (!this.teacherSubjectMapNorm.has(nb))
                                this.teacherSubjectMapNorm.set(nb, []);
                ...
                // ---- Öğretmen adayları ----
                getTeacherCandidates(subject, classroomId) {
                    const list = [];
                    const pinned = subject?.pinnedTeacherByClassroom?.[classroomId];
                    if (pinned && this.teacherById.has(pinned))
                        list.push(pinned);
                    if (Array.isArray(subject?.teacherIds))
                        for (const id of subject.teacherIds)
                            if (this.teacherById.has(id))
                                list.push(id);
                    for (const id of (this.teacherSubjectMap.get(subject?.name) || []))
                        list.push(id);
                    const nb = normalizeName(subject?.name);
                    for (const id of (this.teacherSubjectMapNorm.get(nb) || []))
                        list.push(id);
                ...
                preflightValidate() {
                    ...
                    for (const c of (this.data.classrooms || [])) {
                        ...
                        for (let d = 0; d < DAYS; d++) {
                            ...
                            cap += Math.max(0, w.end - w.start);
                        }
                        const dem = demand.get(c.id) || 0;
                        if (dem > cap)
                            issues.push(`Kapasite aşımı: ${(c.name || c.id)} talep=${dem} > kapasite=${cap}`);
                    }
                    // Validate subject locations exist in data.locations
                    const locIds = new Set((this.data.locations || []).map((l) => l.id));
                    ...
                    if (issues.length)
                        throw new Error('Veri tutarsızlıkları: ' + issues.join(' | '));
                }
                // ---- Çöz ----
                solve() {
                    try {
                        this.preflightValidate();
                    }
                    catch (e) {
                        this.stats.notes.push('Ön kontrol hatası: ' + (e?.message || e));
                        this.finishStats();
                        throw e;
                    }
                    ...
                    while (!this.isTimedOut()) {
                        ...
                        if (ok) {
                            ...
                            if (!this.options.disableLNS) {
                                if (!this.isTimedOut())
                                    this.ruinAndRecreate(20);
                                if (!this.isTimedOut())
                                    this.ruinAndRecreate(25);
                            }
                            // Additional strategy-specific improvements
                            if (this.options.strategy === "sa") {
                                this.simulatedAnnealing();
                                this.compactSchedule();
                                this.stats.notes.push("Simulated Annealing: kabul/ret ile yerel iyileştirme.");
                            }
                            else if (this.options.strategy === "alns") {
                                this.alnsOptimize();
                                this.compactSchedule();
                                this.stats.notes.push("ALNS: uyarlamalı ruin & recreate adımları.");
                            }
                            else if (this.options.strategy === "vns") {
                                this.vnsOptimize();
                                this.compactSchedule();
                                this.stats.notes.push("VNS: farklı komşuluklarda sistematik arama.");
                            }
                            const score = this.objectiveScore();
                            if (score < bestScore) {
                                bestScore = score;
                                bestSnapshot = JSON.stringify(this.schedule);
                                bestNotes = this.stats.notes.slice();
                                foundAny = true;
                            }
                            if (this.options.stopAtFirstSolution) {
                                ...
                                this.finishStats();
                                if (bestNotes && bestNotes.length)
                                    this.stats.notes = Array.from(new Set(bestNotes));
                                return { schedule: this.schedule, stats: this.buildHardestLessons() };
                            }
                            ...
                            if (this.failureReason)
                                this.stats.notes.push(this.failureReason);
                        }
                    }
                    // Finalize
                    ...
                    this.finishStats();
                    if (!this.stats.timedOut && !this.failureReason)
                        this.stats.notes.push('Çözüm bulunamadı. Kısıtlar çok sıkı olabilir.');
                ...
                placeFixedAssignments() {
                    for (const f of (this.data.fixedAssignments || [])) {
                        ...
                        if (d < 0 || d >= DAYS || h < w.start || h >= w.end)
                            return false;
                ...
                buildUnits() {
                    ...
                    for (const subject of (this.data.subjects || [])) {
                        ...
                        for (const classId of (subject.assignedClassIds || [])) {
                            ...
                            for (let d = 0; d < DAYS; d++) {
                                ...
                                while (h < this.maxDailyHours) {
                                    ...
                                    if (a && a.subjectId === subject.id && !counted.has(a)) {
                                        counted.add(a);
                                        let span = 1;
                                        while (h + span < this.maxDailyHours && row[h + span] === a)
                                            span++;
                                        remaining -= span;
                                        h += span;
                                    }
                            ...
                            while (remaining >= 3 && tripleBudget > 0) {
                                out.push({ type: 'single', classroomId: classId, subjectId: subject.id, span: 3 });
                                remaining -= 3;
                                tripleBudget--;
                            }
                            while (remaining >= 2 && doubleBudget > 0) {
                                out.push({ type: 'single', classroomId: classId, subjectId: subject.id, span: 2 });
                                remaining -= 2;
                                doubleBudget--;
                            }
                            for (let i = 0; i < remaining; i++)
                                out.push({ type: 'single', classroomId: classId, subjectId: subject.id, span: 1 });
                        }
                    }
                    for (const g of (this.data.lessonGroups || [])) {
                        let hours = g.weeklyHours || 0;
                        if (hours <= 0)
                            continue;
                        if (g.isBlock) {
                            const pairs = Math.floor(hours / 2);
                            for (let i = 0; i < pairs; i++)
                                out.push({ type: 'group', groupId: g.id, blockSpan: 2 });
                            if (hours % 2 === 1)
                                out.push({ type: 'group', groupId: g.id, blockSpan: 1 });
                        }
                        else {
                ...
                repairLoop(units) {
                    ...
                    while (pool.length) {
                        ...
                        for (let i = 0; i < pool.length; i++) {
                            ...
                            if (cnt < best || (cnt === best && span > bestSpan)) {
                                best = cnt;
                                bestSpan = span;
                                idx = i;
                            }
                ...
                drainDeferred(pool) {
                    if (this._deferredReinsert && this._deferredReinsert.length) {
                        for (const u of this._deferredReinsert)
                            pool.push(u);
                        this._deferredReinsert = [];
                    }
                }
                // Tekil + küçük sök-tak
                placeUnitSingleWithRepair(u) {
                    ...
                    if (!victims.length)
                        return false;
                    const removed = [], removedAssn = new Set();
                    for (const v of victims) {
                        if (this.fixedMask[v.classroomId]?.[v.day]?.[v.hour])
                            continue;
                        const assn = this.schedule[v.classroomId]?.[v.day]?.[v.hour];
                        if (!assn || removedAssn.has(assn))
                            continue;
                    ...
                    for (const r of removed) {
                        const s2 = this.subjectById.get(r.subjectId);
                        if (!s2)
                            continue;
                        const tlist = this.getTeacherCandidates(s2, r.classroomId).map(id => this.teacherById.get(id)).filter(Boolean);
                        const cls2 = this.classroomById.get(r.classroomId);
                        if (!cls2)
                            continue;
                        const cR = this.enumerateValidSlotsForSingle(r, s2, cls2, tlist);
                        if (cR.length) {
                            const top = rng.pick(cR);
                            this.placeLesson(r.classroomId, s2, top.teacher, top.d, top.h, r.span);
                        }
                    }
                    return false;
                }
                // Grup (varsa)
                placeUnitGroupWithRepair(unit) {
                ...
                tryPlaceGroupAtAnyValidSlot(group, subject, span) {
                    ...
                    for (let d = 0; d < DAYS; d++) {
                        ...
                        for (let h = win.start; h < maxH; h++) {
                            ...
                            if (assign) {
                                this.placeGroupLesson(group, subject, d, h, assign, span);
                                return true;
                            }
                        }
                    }
                ...
                countValidPlacementsApprox(unit) {
                    if (unit.type === 'group') {
                        const group = (this.data.lessonGroups || []).find(g => g.id === unit.groupId);
                        const subject = this.subjectById.get(group?.subjectId);
                        if (!group || !subject)
                        ...
                        for (let d = 0; d < DAYS; d++) {
                            ...
                            const maxH = win.end - (span - 1);
                            for (let h = win.start; h < maxH; h++)
                                if (this.findGroupAssignment(group, subject, d, h, teachers, span))
                                    cnt++;
                        }
                        return cnt;
                    }
                        ...
                        for (const t of teachers) {
                            for (let d = 0; d < DAYS; d++) {
                                ...
                                for (let h = win.start; h < maxH; h++)
                                    if (this.isValid(unit.classroomId, subject, t, d, h, unit.span))
                                        cnt++;
                            }
                        }
                        return cnt;
                ...
                enumerateValidSlotsForSingle(unit, subject, classroom, teachers) {
                    const out = [];
                    ...
                    for (const t of teachers) {
                        for (let d = 0; d < DAYS; d++) {
                            ...
                            const maxH = win.end - (span - 1);
                            for (let h = win.start; h < maxH; h++)
                                if (this.isValid(unit.classroomId, subject, t, d, h, span))
                                    out.push({ d, h, teacher: t });
                        }
                    }
                    return out;
                }
                pickBlockingAssignmentsForSingle(unit, subject, classroom, teachers, K) {
                    const blocks = [];
                    const span = unit.span || 1;
                    for (const t of teachers) {
                        for (let d = 0; d < DAYS; d++) {
                    ...
                    const best = blocks[0].items.filter((x) => !this.fixedMask[x.classroomId]?.[x.day]?.[x.hour]);
                    return best.slice(0, K || 6);
                }
                listConflictingAssignments(classroomId, subject, teacher, day, hour, span) {
                    ...
                    if (hour < start || (hour + span) > end)
                        return null;
                    if (classroom.level === 'Ortaokul' && !teacher.canTeachMiddleSchool)
                        return null;
                    if (classroom.level === 'Lise' && !teacher.canTeachHighSchool)
                        return null;
                    const items = [];
                    const row = this.schedule[classroomId][day];
                    let left = 0, right = 0;
                    for (let i = hour - 1; i >= start; i--) {
                        const a = row[i];
                        if (a && a.subjectId === subject.id)
                            left++;
                        else
                            break;
                    }
                    for (let i = hour + span; i < end; i++) {
                        const a = row[i];
                        if (a && a.subjectId === subject.id)
                            right++;
                        else
                    ...
                    if (left + span + right > maxC) {
                        for (let i = hour - 1; i >= start; i--) {
                            const a = row[i];
                            if (a && a.subjectId === subject.id)
                                items.push({ classroomId, day, hour: i });
                            else
                                break;
                        }
                        for (let i = hour + span; i < end; i++) {
                    ...
                    for (let k = 0; k < span; k++) {
                        const h = hour + k;
                        if (!teacher.availability?.[day]?.[h])
                            return null;
                        const assnC = this.schedule[classroomId][day][h];
                        if (assnC)
                            items.push({ classroomId, day, hour: h });
                        if (this.teacherOccupied[teacher.id][day][h]) {
                            const clash = this.findAssignmentByTeacherAt(teacher.id, day, h);
                            if (clash)
                                items.push({ classroomId: clash.classroomId, day, hour: clash.hour });
                        }
                        if (subject.locationId && this.locationOccupied[subject.locationId]?.[day]?.[h]) {
                            const clash = this.findAssignmentByLocationAt(subject.locationId, day, h);
                            if (clash)
                                items.push({ classroomId: clash.classroomId, day, hour: clash.hour });
                        }
                    }
                    ...
                    for (const it of items) {
                        const k = it.classroomId + '|' + it.day + '|' + it.hour;
                        if (!seen.has(k)) {
                            seen.add(k);
                            uniq.push(it);
                        }
                ...
                findAssignmentByTeacherAt(teacherId, day, hour) {
                    for (const classId of Object.keys(this.schedule)) {
                        ...
                        if (Array.isArray(a.teacherIds) && a.teacherIds.includes(teacherId))
                            return { classroomId: classId, day, hour };
                        if (a.teacherId === teacherId)
                            return { classroomId: classId, day, hour };
                    }
                    return null;
                }
                findAssignmentByLocationAt(locationId, day, hour) {
                    for (const classId of Object.keys(this.schedule)) {
                        const a = this.schedule[classId][day][hour];
                        if (a && a.locationId === locationId)
                            return { classroomId: classId, day, hour };
                    }
                    return null;
                }
                getGroupAllowedWindow(classIds, day) {
                    let start = 0, end = this.maxDailyHours;
                    for (const cid of classIds) {
                        const c = this.classroomById.get(cid);
                        if (!c)
                            continue;
                ...
                findGroupAssignment(group, subject, day, hour, teacherIds, span) {
                    const win = this.getGroupAllowedWindow(group.classroomIds, day);
                    if (hour < win.start || (hour + span) > win.end)
                        return false;
                    for (const classId of group.classroomIds) {
                        const c = this.classroomById.get(classId);
                        if (!c)
                            return false;
                        const w = this.getAllowedWindow(c, day);
                        if (hour < w.start || (hour + span) > w.end)
                            return false;
                        for (let k = 0; k < span; k++)
                            if (this.schedule[classId][day][hour + k])
                                return false;
                        if (this.violatesRunLimit(classId, subject.id, day, hour, span))
                            return false;
                    }
                    for (let k = 0; k < span; k++)
                        if (subject.locationId && this.locationOccupied[subject.locationId]?.[day]?.[hour + k])
                            return false;
                    ...
                    for (const classId of classes) {
                        ...
                        for (const t of teacherObjs) {
                            let ok = true;
                            if ((cr.level === 'Ortaokul' && !t.canTeachMiddleSchool) || (cr.level === 'Lise' && !t.canTeachHighSchool))
                                ok = false;
                            for (let k = 0; k < span && ok; k++)
                                if (!t.availability?.[day]?.[hour + k] || this.teacherOccupied[t.id][day][hour + k])
                                    ok = false;
                            if (ok)
                                list.push(t);
                        }
                    ...
                    classes.sort((a, b) => (candByClass.get(a).length - candByClass.get(b).length));
                    const used = new Set();
                    const assign = {};
                    const dfs = (i) => {
                        if (i === classes.length)
                ...
                // ---- Geçerlilik ----
                violatesRunLimit(classroomId, subjectId, day, hour, span) {
                    const classroom = this.classroomById.get(classroomId);
                    const { start, end } = this.getAllowedWindow(classroom, day);
                    const row = this.schedule[classroomId][day];
                    let left = 0, right = 0;
                ...
                isValid(classroomId, subject, teacher, day, hour, span) {
                    // Count every feasibility check as an attempt for reporting
                    this.stats.attempts++;
                    const classroom = this.classroomById.get(classroomId);
                    const { start, end } = this.getAllowedWindow(classroom, day);
                    if (hour < start || (hour + span) > end) {
                        this.stats.invalidReasons.blockBoundary++;
                        return false;
                    }
                    if (classroom.level === 'Ortaokul' && !teacher.canTeachMiddleSchool) {
                        this.stats.invalidReasons.levelMismatch++;
                        return false;
                    }
                    if (classroom.level === 'Lise' && !teacher.canTeachHighSchool) {
                        this.stats.invalidReasons.levelMismatch++;
                    ...
                    for (let k = 0; k < span; k++) {
                        const h = hour + k;
                        if (this.fixedMask[classroomId]?.[day]?.[h])
                            return false;
                        if (!teacher.availability?.[day]?.[h]) {
                            this.stats.invalidReasons.availability++;
                            return false;
                        }
                        if (this.schedule[classroomId][day][h]) {
                            this.stats.invalidReasons.classBusy++;
                            return false;
                        }
                        if (this.teacherOccupied[teacher.id][day][h]) {
                        ...
                        if (subject.locationId && this.locationOccupied[subject.locationId]?.[day]?.[h]) {
                            this.stats.invalidReasons.locationBusy++;
                            return false;
                        }
                    }
                    if (this.violatesRunLimit(classroomId, subject.id, day, hour, span)) {
                        this.stats.invalidReasons.blockBoundary++;
                        return false;
                    }
                    return true;
                ...
                // ---- Place/Remove ----
                placeLesson(classroomId, subject, teacher, day, hour, span) {
                    // Create assignment using teacherIds array to support multi-teacher assignments
                    const a = { subjectId: subject.id, teacherIds: [teacher.id], classroomId, locationId: subject.locationId };
                    for (let k = 0; k < span; k++) {
                ...
                removeLesson(classroomId, assignment, day, hour, span) {
                    if (!assignment)
                        return;
                            ...
                            if (assignment?.teacherIds && Array.isArray(assignment.teacherIds)) {
                                for (const tid of assignment.teacherIds) {
                                    if (this.teacherOccupied[tid])
                                        this.teacherOccupied[tid][day][idx] = false;
                                }
                            }
                            else if (assignment.teacherId) {
                                const tid = assignment.teacherId;
                                if (this.teacherOccupied[tid])
                                    this.teacherOccupied[tid][day][idx] = false;
                            }
                            if (subject?.locationId && this.locationOccupied[subject.locationId])
                                this.locationOccupied[subject.locationId][day][idx] = false;
                            this.dailyLessonCounts[classroomId][day]--;
                        }
                    }
                }
                getSpanAt(classId, day, startHour) {
                    const row = this.schedule[classId][day];
                    const a = row[startHour];
                    if (!a)
                        return 1;
                    let span = 1;
                    while (startHour + span < row.length && row[startHour + span] === a)
                        span++;
                    return span;
                }
                placeGroupLesson(group, subject, day, hour, teacherAssignments, span) {
                    for (const classId of group.classroomIds) {
                    ...
                    while (changed) {
                        ...
                        for (const classId of Object.keys(this.schedule)) {
                            ...
                            for (let d = 0; d < DAYS; d++) {
                                ...
                                let h = start;
                                while (h < end) {
                                    if (this.schedule[classId][d][h] !== null || this.fixedMask[classId][d][h]) {
                                        h++;
                                        continue;
                                    }
                                    let k = h + 1;
                                    while (k < end && this.schedule[classId][d][k] === null && !this.fixedMask[classId][d][k])
                                        k++;
                                    if (k >= end)
                                        break;
                                    const assn = this.schedule[classId][d][k];
                                    if (!assn) {
                                        h = k + 1;
                                        continue;
                                    }
                                    const subject = this.subjectById.get(assn.subjectId);
                                    const primaryTid = Array.isArray(assn.teacherIds) ? assn.teacherIds[0] : assn.teacherId;
                                    const teacher = this.teacherById.get(primaryTid);
                                    if (!subject || !teacher) {
                                        h++;
                                        continue;
                                    }
                ...
                countOverConsecForDay(classId, day) {
                    const row = this.schedule[classId][day];
                    let pen = 0, h = 0;
                    while (h < row.length) {
                        const a = row[h];
                        ...
                        let span = 1;
                        while (h + span < row.length && row[h + span] === a)
                            span++;
                ...
                teacherSpreadPenalty(day) {
                    ...
                    for (const tId of Object.keys(this.teacherOccupied)) {
                        ...
                        if (last >= first && Number.isFinite(first))
                            p += Math.max(0, (last - first + 1) - slots);
                    }
                    return p;
                }
                objectiveScore() {
                ...
                ruinAndRecreate(maxRemove) {
                    ...
                    for (const classId of Object.keys(this.schedule)) {
                        ...
                        for (let d = 0; d < DAYS; d++) {
                            ...
                            for (let h = start; h < end; h++) {
                                ...
                                if (!a)
                                    continue;
                                if (h > start && this.schedule[classId][d][h - 1] === a)
                                    continue;
                                const span = this.getSpanAt(classId, d, h);
                                const primaryTidA = Array.isArray(a.teacherIds) ? a.teacherIds[0] : a.teacherId;
                    ...
                    if (newScore <= baseScore)
                        return true; // accept non-worse
                    // Rollback if worse
                    this.schedule = JSON.parse(snapshot);
                    return false;
                }
                localBlockCost(classId, day, hour, span, teacherId) {
                        ...
                        for (let d = 0; d < DAYS; d++) {
                            ...
                            for (let h = start; h < end; h++) {
                                ...
                                if (!a)
                                    continue;
                                if (h > start && this.schedule[classId][d][h - 1] === a)
                                    continue; // blok başı değil
                                const span = this.getSpanAt(classId, d, h);
                                const subject = this.subjectById.get(a.subjectId);
                                const teacher = this.teacherById.get(Array.isArray(a.teacherIds) ? a.teacherIds[0] : a.teacherId);
                    ...
                    for (const classId of Object.keys(this.schedule)) {
                        ...
                        for (let d = 0; d < DAYS; d++) {
                            ...
                            for (let h = start; h < end; h++) {
                                ...
                                const teachA = this.teacherById.get(Array.isArray(A.teacherIds) ? A.teacherIds[0] : A.teacherId);
                                for (let k = h + spanA; k < end; k++) {
                                    const B = this.schedule[classId][d][k];
                                    if (!B)
                                        continue;
                                    if (k > h + spanA && this.schedule[classId][d][k - 1] === B)
                                        continue;
                                    const spanB = this.getSpanAt(classId, d, k);
                                    const subjB = this.subjectById.get(B.subjectId);
                                    const teachB = this.teacherById.get(Array.isArray(B.teacherIds) ? B.teacherIds[0] : B.teacherId);
                ...
                applyMove(m) {
                    if (m.type === 'relocate') {
                        ...
                        const teach = this.teacherById.get(Array.isArray(a.teacherIds) ? a.teacherIds[0] : a.teacherId);
                ...
                reverseMove(m) { this.applyMove(m); } // swap kendi tersi; relocate tersi de aynı apply ile geri alınır
                tabuOptimize() {
                ...
                simulatedAnnealing() {
                    ...
                    for (let iter = 0; iter < maxIter && !this.isTimedOut(); iter++) {
                        ...
                        if (!accept)
                            ...
                            if (next < bestScore) {
                                bestScore = next;
                                best = JSON.stringify(this.schedule);
                            }
                        }
                        T *= cooling;
                        if (T < 1e-4)
                ...
                alnsOptimize() {
                    ...
                    for (let i = 0; i < iters && !this.isTimedOut(); i++) {
                        ...
                        let improved = false;
                        if (op === 'small')
                            improved = !!this.ruinAndRecreate(12);
                        else if (op === 'big')
                            improved = !!this.ruinAndRecreate(28);
                        else {
                        ...
                        if (!improved)
                            this.schedule = JSON.parse(snapshot);
                        if (op === 'small')
                            wSmall += improved ? 0.2 : -0.05;
                        else if (op === 'big')
                            wBig += improved ? 0.2 : -0.05;
                        else
                            wReloc += improved ? 0.2 : -0.05;
                        wSmall = Math.max(0.1, wSmall);
                        wBig = Math.max(0.1, wBig);
                        wReloc = Math.max(0.1, wReloc);
                    }
                    this.schedule = JSON.parse(best);
                }
                // ---- Simple VNS ----
                ...
                // ---- Puanlama ----
                scorePlacement(lesson, d, h, teacher) {
                    let score = 0;
                    ...
                    for (let x = start; x < end; x++) {
                        const a = row[x];
                        if (a?.subjectId === lesson.subjectId) {
                            same++;
                            if (x + 1 < end && row[x + 1] === a)
                                hasBlock = true;
                        }
                    }
                    score -= same * 10;
                    if ((lesson.span || 1) > 1 && hasBlock)
                        score -= 14;
                    let tload = 0;
                    for (let x = start; x < end; x++)
                        if (this.teacherOccupied[teacher.id][d][x])
                            tload++;
                    if (tload > 5)
                        score -= (tload - 5);
                    if (span === 3)
                    ...
                    if (pinnedId && pinnedId === teacher.id)
                        score += 15;
                    return score;
                }
                // ---- Grup yardımcıları & istatistik ----
                pickBlockingAssignmentsForGroup(group, subject, span, K) {
                    let best = null;
                    for (let d = 0; d < DAYS; d++) {
                        const win = this.getGroupAllowedWindow(group.classroomIds, d);
                        const maxH = win.end - (span - 1);
                        for (let h = win.start; h < maxH; h++) {
                            ...
                            for (const classId of group.classroomIds) {
                                ...
                                const w = this.getAllowedWindow(c, d);
                                if (h < w.start || (h + span) > w.end) {
                                    feas = false;
                                    break;
                                }
                                for (let k = 0; k < span; k++) {
                                    const a = this.schedule[classId][d][h + k];
                                    if (a)
                                        blockers.push({ classroomId: classId, day: d, hour: h + k });
                                }
                            }
                            if (!feas)
                                continue;
                            if (best === null || blockers.length < best.blockers.length)
                                best = { d, h, blockers };
                        }
                    }
                    if (!best)
            ...
            try {
                ...
                if (solver._deferredReinsert && solver._deferredReinsert.length && result?.schedule) {
                    ...
                    if (ok2) {
                        ...
                        if (solver.options.strategy === 'tabu') {
                            solver.tabuOptimize();
                            solver.compactSchedule();
                        }
                        solver.finishStats();
...
const solveTimetableLocally = (data, options) => {
    return new Promise((resolve) => {
        const workerScript = makeWorkerScript();
        // In Node smoke-test environment: evaluate the worker script in-process using a fake `self`.
        // This bypasses browser Blob/Worker/URL complexities and runs the solver synchronously/asynchronously
        // inside the same process. The worker code uses `self.postMessage` to send result — we capture that.
        let worker = null;
        try {
            const fakeSelf = { onmessage: null, postMessage: (msg) => {
                    // emulate worker's postMessage to main thread
                    if (worker && typeof worker.onmessage === 'function')
                        worker.onmessage({ data: msg });
                } };
            // expose `self` so that the worker script attaches handlers to it
            globalThis.self = fakeSelf;
            // evaluate the script (it should set self.onmessage handler)
            eval(workerScript);
            // create a light wrapper to mimic worker API used below
            worker = {
                onmessage: null,
                postMessage: (m) => { try { if (typeof fakeSelf.onmessage === 'function') fakeSelf.onmessage({ data: m }); } catch (e) { /* ignored */ } },
                terminate: () => { try { delete globalThis.self; } catch (_) { } }
            };
        }
        catch (e) {
            // Fallback: resolve with failure stats
            resolve({ schedule: null, stats: { startedAt: Date.now(), endedAt: Date.now(), elapsedSeconds: 0, notes: ['Worker eval failed: ' + (e?.message || e)] } });
            return;
        }
        ...
        worker.onmessage = (event) => {
            ...
            if (success) {
                resolve({ schedule: schedule, stats: stats });
            }
            else {
                const finalStats = stats || defaultEmptyStats();
                if (error && !finalStats.notes.includes(error))
                    finalStats.notes.push(error);
                if (finalStats.notes.length === 0)
                    finalStats.notes.push('Bilinmeyen bir hata nedeniyle çözüm bulunamadı.');
...
exports.solveTimetableLocally = solveTimetableLocally;
